...
BASH_RULES = [
    ...
    (rf"{_CMD}(ipconfig|hostname|whoami|systeminfo)\b",
                                              "none", "system info"),
    (rf"{_CMD}es(\.exe)?\s",                  "none", "Everything CLI (es.exe — read-only file search)"),
    (rf"{_CMD}where(\.exe)?\s",               "none", "where (Windows — locate executable in PATH)"),
    (rf"{_CMD}fc(\.exe)?\s",                  "none", "fc (Windows — file compare)"),
    (rf"{_CMD}findstr(\.exe)?\s",             "none", "findstr (Windows — read-only text search)"),
