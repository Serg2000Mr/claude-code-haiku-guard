...
> ℹ️ **Alternative models.** If you can't use Claude (region restriction, preference, cost), swap in Mistral / DeepSeek / Qwen / Llama via `HAIKU_GUARD_MODEL`. See [SETUP.md § Choosing a model](SETUP.md#choosing-a-model) for a candidate list. Comparative benchmarking has not been done yet — expect some cases to flip on switch.
